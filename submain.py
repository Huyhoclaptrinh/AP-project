...
def books_list_win():
    ...
    # Labels
    lbl_book_name = Label(books_list, text="Book's name")
    lbl_book_name.grid(row=0, column=0)

    lbl_author_name = Label(books_list, text="Author")
    lbl_author_name.grid(row=1, column=0)

    lbl_book_type = Label(books_list, text="Book's type")
    lbl_book_type.grid(row=0, column=2)

    lbl_quantity = Label(books_list, text="Quantity")
    lbl_quantity.grid(row=1, column=2)

    lbl_price = Label(books_list, text="price")
    lbl_price.grid(row=2, column=0)

    # Entries
    e1 = Entry(books_list, textvariable=book_name)
    e1.grid(row=0, column=1)

    e2 = Entry(books_list, textvariable=author_name)
    e2.grid(row=1, column=1)

    e3 = Entry(books_list, textvariable=book_type)
    e3.grid(row=0, column=3)

    e4 = Entry(books_list, textvariable=quantity)
    e4.grid(row=1, column=3)

    e5 = Entry(books_list, textvariable=price)
    e5.grid(row=2, column=1)

    # list box
    ls1 = Listbox(books_list, height=15, width=60)
    ls1.grid(row=3, column=0, columnspan=5)

    # Button
    bn1 = Button(books_list, text="Add", width=12, command=record)
