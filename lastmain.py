...
def revenue_win():
    ...
    listBox.bind('<Double-Button-1>',GetValue)
                                         ...
                                         # TURNOVER PLOT WINDOW
                                             ...
                                             # USER WINDOW
def user_win(): # đây là phần user window 
    global user
    user = Tk()
...
def login_sucess(): # sửa nút button
    global screen3
    #screen3 = Toplevel(window)
    #screen3.title("Success")
    #screen3.geometry("150x100")
    #l1 = Label(screen3,text="Log in sucess!").pack()
    #b1=Button(screen3, text="OK", command=user_menu_win).pack()
    messagebox.showinfo("","success!",command=user_menu_win())
    ...
    #messagebox.showinfo("","Log in sucess!")
    ## các lệnh thực thi để tắt các cửa sổ thừa
    #window.iconify()#win chính tự thu về dạng icon
    #login_sucess.close()
    #user_login_win.close()
    #user.close()
...
#	# Open our image
#	bg1 = tk.PhotoImage(file= "D:\Ri cọt\WinBG.png")
#	# Resize the image
...
#Define image
bg_image = tk.PhotoImage(file= "D:\Ri cọt\WinBG.png")
...
#l1=Label(window,image=bg_image)
#l1.place(x=0, y = 0,relwidth=1, relheight=1)

l2=Label(window,text="BOOKSTORE MANAGEMENT0",font="times 20")
